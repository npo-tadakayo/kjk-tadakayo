// 認定事業者向け書類（協定書・説明資料）のHTML生成。タダカヨ ブランドカラー（赤系）。
...
export function agreementHTML(p, s = {}){
  ...
  return `${styleBlock()}<div class="doc">
    ...
    <div class="inner">
      ...
      ${A(9,"知的財産・商標","甲は乙に「タダサポ」「タダカヨ」「認定事業者」の名称・ロゴの非独占的使用を本プログラムの範囲で許諾する。「認定」は任意認定であり公的資格でない旨を誤認させない表示とする。")}
...
export function briefingHTML(p, s = {}){
  ...
  return `${styleBlock()}<div class="doc">
    <div class="band"><div class="host">${HOST.name} ／ ${HOST.biz}</div><h1>介護情報基盤 伴走支援パートナー（認定事業者）募集のご案内</h1></div>
    <div class="inner">
      ${to ? `<div class="cover-to">${to}</div>` : ""}
      <p class="muted">介護現場が取り残されないために、地域で導入を支える仲間（認定事業者）を募集しています。エンド事業所は自己負担¥0で導入でき、認定事業者は伴走支援で収益化できます。</p>
      ...
      <p class="muted">カードリーダー卸価格は AB Circle 仕入と同額（パススルー）：USB 約¥3,490／BT 約¥7,520（数量帯で変動・税別）。伴走支援費：1台¥60,000／2台¥55,000／3台¥50,000。任意でタダカヨへ寄付（活動支援）。</p>

      <h2>8. 認定までの流れ</h2>
      <p>① お申し込み → ② 面談 → ③ 研修受講＋確認テスト → ④ 協定書の締結 → ⑤ 認定証・発注アカウント発行 → ⑥ 活動開始</p>

      <h2>9. 認定事業者の基準（安心のための約束）</h2>
