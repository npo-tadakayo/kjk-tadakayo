...
- 発行元の**住所・TEL・担当者名**（設定画面に項目を足す必要がある）
- 品名の下の**説明行**（「導入設定・マイナ資格確認アプリ設定・…」）
- 表題を「領収**証**」にするか（現行は「領収書」）
...
- **認可ドメインに `kjk-tadakayo-admin--loc-0912-ny2ptwh2.web.app` を足したまま**
  （プレビューでログインして検証するため）。検証が済んだら外す。
  ⚠️ 過去のプレビュー用ドメインが**9件たまっている**ので、まとめて掃除すると良い
