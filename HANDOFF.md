...
|---|---|---|
| 1 | **Formspree の扱い** → **PMS報告書を作成済み**（`docs/PMS報告_LP問い合わせフォームの外部送信_20260908.md`／PDF は `00_outbox/介護情報基盤_メール送信元変更とPMS報告_20260908/`）。規約を確認したところ **Formspree は「独立したデータ管理者」を名乗り、送信データを自社のマーケティングにも使う**と書かれており、単純な委託として整理するのは難しい。**外国にある第三者への提供**として扱うのが安全 | 止める → `index.html` の Formspree 送信を外して lp 再デプロイ（30分）／ 残す → 台帳・ポリシー・本人同意の設計 |
