...
16. **研修動画 単元5 を v1.2 に作り直し**（呼称「認定事業者」・ナレーション9本再生成・CRM画面7枚撮り直し・実データはサンプル化）。共有ドライブ `研修動画/CRM操作説明_全7単元/` に配置、v1.0 は `_旧版_20260906/`、`00_索引.md` 更新。素材フォルダは `06_介護情報基盤事業/動画_CRM単元5_認定事業者_20260827/`（改名）。**単元2・3 への Web申込の場面追加は未着手**（改訂メモ参照）
...
-0.4. **統合 Phase 1・2 は完了・本番稼働**（2026-09-06・上記13〜14）。残り: **研修動画 単元2・3 に Web申込の場面を追加**（台本の書き足し→撮影。`動画_CRM単元5_認定事業者_20260827/改訂メモ_20260906.md` 末尾）／計画書 Phase 3「スタッフが見積もりを作る」は未着手
