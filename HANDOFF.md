...
- **条文はドラフト1.0のまま**（admin/js/consent-document.js）。次田さんの文言確定待ち。
  変えるときは version を上げて旧版を CONSENT_ARCHIVE に凍結（ケアプー docs/19 の作法）
...
- ⚠ 単元7動画は「事前確認は8項目のカードから」の説明のまま（承諾書カードが上に増えた）。
  単元6も「閲覧のみ」の説明が権限修正前の実態。どちらも作り直しは次田さんの指示待ち
