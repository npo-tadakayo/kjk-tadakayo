...
| 2026-09-06 | **統合 Phase 1・2 を本番反映し、計画書 §4.4 の残りを実装**（`fa13e71`）: 案件一覧に `quoteFilter`（`latestQuoteId`／`orderedAt`／`orderedVia==="web"` で判定・`cases.js quoteMatch()`）／`supply.js prefillShipFromCase()` は `shipments where caseId==X && status=="draft"` があれば `openShip(existing)` で開く（Web申込の自動下書きの二重作成防止）／`case-detail.js mergeInto()` の付け替え対象に `quotes`・`shipments` を追加し、`latestQuoteId`/`quoteIssuedAt`/`orderedAt`/`orderedVia` は残る側に無いときだけ引き継ぐ／流入元ラベルを「見積もり作成」に統一（`report.js` の独自 SOURCE_LABELS 含む）。**F5**（`4c4555e`）: `mitsumori.html loadLeadToken()` が郵便番号を入れると `wireZipAutofill` の lookup が走り、引き継いだ `city` を上書きして区が消えていた → `wireZipAutofill` が `{markLookedUp}` を返し、引き継ぎ時にその番号を検索済み扱いにする。**本番で通し確認2回**（#142/#143・EST-2026-0008/0009・SH-2026-0009/0010・すべて削除済み）。admin live `fc76dde1caf7bf4b`→`d4077176be22bc6a`（manual）／lp live `622b6548ce4c8ad2`。研修動画 単元5 v1.2（撮影は html2canvas→Storage `reports/` 経由・`06_介護情報基盤事業/動画_CRM単元5_認定事業者_20260827/改訂メモ_20260906.md`） |
| 2026-09-06 | **動画マニュアル（CRM内再生）**: `admin/videos.html` + `js/videos.js`（一覧・`?v=U5` で選択）／`js/video-player.js`（`mountVideo`・`openVideoModal`・`[data-video]` ボタンの自動結線・スタイル注入）／`js/video-catalog.js`（目録 SSOT: id/no/title/duration/version/file）。動画・字幕は **Storage `training/crm-u{n}.mp4|.vtt`**（storage.rules: `@tadakayo.jp` read のみ・write false）。`getDownloadURL` で取得し `<video crossorigin="anonymous">`＋`<track kind="subtitles">`。**バケットに CORS を設定**（origin: admin.web.app / firebaseapp.com / admin.kjk.tadakayo.jp。字幕トラックと Range 取得に必須）。mp4 は `-movflags +faststart` で moov を先頭へ（元ファイルは末尾だった）。差し替えは `scripts/sync-training-videos.py`（srt→vtt・faststart・upload）。sidebar.js HELP に `/videos.html`。⚠️ **Chrome は非表示タブの動画読み込みを保留する**ため、拡張経由の自動検証では readyState 0 のまま＝配信は curl/fetch の 206 で確かめる。admin live `d7f6acee400ffaa1`（23:00 JST） |
