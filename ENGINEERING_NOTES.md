...
| 2026-06-05 | セキュリティ続報: H-4 は全SA（appspot/gmail-sa/compute）で USER_MANAGED 鍵ゼロ＝**実態クリア**と判明（キーレスDWD運用・追加対応不要）。H-3 は6関数が compute SA（roles/editor＋aiplatform.user）共有を確認。M-5 は通知ch 0件のため準備のみ。SECURITY_REMEDIATION.md（開発チーム向け対応状況レポート）作成 |
