...
# セッション⑮・⑭の詳細（2026-08-01 にHANDOFFから移動）
...
# 2026-09-06 に HANDOFF.md から移設（セッション⑯〜㉓・2026-07-30〜08-27）

## 今セッション（㉓・2026-08-26〜27）でやったこと

### 1. マニュアルを最新化し、担当の呼び方を統一（`fc35123`・**本番反映済み**）

- 🔴 **マニュアルが 2026-08-12 で止まっており、8/26 に入れた3機能が1語も書かれていなかった**
  （「紹介元」「担当営業」「統合」等すべて0件）。`MANUAL.md` と `admin/manual.html` の**両方**に追記
  （使い方4見出し／FAQ2問／変更履歴）。**片方だけの更新は過去に食い違いを生んでいるので必ず両方**
- 🔴 **同じ言葉が別人を指していた**: 一覧の列見出し「担当者」は**うちの営業担当**なのに、
  同じ画面の「担当者名」は**事業所側の担当者**。CSVは「担当スタッフ」でまた別。
  **画面・CSV・マニュアルを「担当営業」に統一**（「担当スタッフ」0件を確認）。
  検索欄の説明は実際の検索対象に合わせて「担当者名」に

### 2. CRM操作説明の動画を作成（3分59秒・v1.1）

- 配布: `00_outbox/CRM営業担当と紹介元_20260826/`（旧版は `_旧版/`）／制作素材: `06_介護情報基盤事業/動画_CRM営業担当と紹介元_20260826/`
- 内容は営業担当の割り当て・紹介元・統合時の食い違い選択の3手順。**実在の事業所名は映していない**
  （サンプル案件 #9001/#9002 を作って撮影し、撮影後に対象外へ片付け済み）

### 3. 説明動画シリーズ（全6単元）を開始

- 単元構成は次田さん承認済み。**単元1「はじめに・全体像」の台本まで完成**（4分13秒・9枚）
- 🔴 **詳細と撮影の作法は `06_介護情報基盤事業/ガイドブック動画_パイプライン/HANDOFF.md` の
  「CRM説明動画シリーズ」節が正本**。ここから再開する
- **次の判断（未決）**: 単元1を先に完成させるか、6単元の台本を先に全部作るか。
  司令塔の推奨は後者（単元をまたぐ内容の重複を先に調整できる）

### 4. Functions を2本デプロイ

`webhookLpInquiry` / `webhookMitsumori`。これで**全3経路の案件が `referralSource` を持つ**

## 前セッション（㉒・2026-08-26）でやったこと

### 1. 案件に「営業担当」と「紹介元」を追加（`2a3ca82`・**本番反映済み**）

次田さん要望。**当初ケアプー記録に実装してしまい、途中で対象を取り違えていたことが判明して作り直した**（ケアプー記録側の実装は破棄済み・残っていない）。対象は**このCRM（tadakayo-kiban）の `cases`**。

- 🔴 **営業担当は「表示はあるが割り当てる操作が存在しない」状態だった**（`assignedUserId`/`assignedUserName` は作成時 null のまま、一覧とカンバンは表示のみ）。案件詳細から選べるようにしたのが今回の本体
  - 保存は **ID と氏名の両方**（片方だけだと一覧に古い名前が残る）。ID には `users` の文書ID＝**メールアドレス**を使用（他に安定したIDが無いため。将来 uid に変えるなら移行が要る）
  - 変更は**タイムラインに記録**。無効化済みユーザーが割当済みなら「（無効）」付きで候補に残す
  - **権限制御は入れていない**（staff でも変更可）。管理者限定にするなら1行
- **紹介元**（`cases.referralSource`）: 初期値 **タダカヨ / 直接（飛び込み）**。案件詳細で選択・一覧の列と絞り込み（「未設定」も）・カンバン表示・CSV。**設定画面で追加/改名/並び替え/無効化**（削除は不可＝過去案件の表示を壊さないため）
  - ⚠️ **既存の `source`（lp_inquiry / mitsumori_quote / manual＝流入経路）とは別物**。あちらはダッシュボード・レポートの集計キーなので拡張していない
- **統合時の食い違い選択**: 担当営業・紹介元・担当者名・メール・電話・法人名が食い違う場合にモーダルで採用側を選ぶ。片方だけに値があるときは従来どおり自動補完。**残る案件＝緑／消える案件＝赤**で案件番号と事業所名を明示
- Functions 2本（`webhookLpInquiry` / `webhookMitsumori`）も **2026-08-26 デプロイ済み**＝全3経路で `referralSource: null` を明示

### 2. LPに 8/26 厚労省説明会のお知らせを掲載（`e08cc0a`・**本番反映済み**）

- トップのヒーロー直下に `<section id="news">`。**掲載終了時はこのセクションごと削除してよい**（CSSにもコメントあり）
- 出典は**厚労省PDF2本・YouTube・介護情報基盤ポータル**だけに限定。説明会資料の公式URL `https://www.mhlw.go.jp/content/12306000/001742527.pdf`（42p・手元PDFとバイト数一致＝原本）
- 🔴 **「令和9年1月12日に切替」「並行利用の期間は設けない」は絶対に公開しない**。全国大会会場での口頭情報で非公表、しかも**8/26の説明会では「並行期間を経てから終了」と説明されており内容が逆**
- 🔴 **社内理解の訂正**: 従来「並行利用なし」と理解していたが、国は**「並行期間を経てから最終的に終了」**と明言（録音・文字起こしが根拠）。伴走支援の説明資料に影響する可能性あり
- 具体的な移行日は**今後、別途の事務連絡で通知予定**。操作・留意点の説明会も別途検討中

### 3. そのほか

- **解説動画 PART 0〜3（計39分）** を共有ドライブ `TM_介護情報基盤を広め隊/02_マニュアル・ガイドブック/研修動画/` へ配置。ガイドブックPDFも Ver.2026-1.1 に差し替え（表紙・奥付とも 1.0 のままだったのを修正）
- **介護情報基盤スペースのChat webhook を Secret Manager へ登録**（`carepoo-tadakayo` の `TADAKAYO_CHAT_WEBHOOK_KIBAN`）。`_tools/chat_post.py --space kiban` で投稿できる。**URLは会話・コード・メモリに書かない**
- 動画の周知とCRM新機能の周知を同スペースへ投稿済み

## 前セッション（㉑・2026-08-13）でやったこと
...
# 2026-09-13 に HANDOFF.md から移した分（2026-09-01〜09-11 の完了済み）
...
18b. **本日出した分（Hosting 14回・Functions 9本）を総点検**（9/7 未明・LP 18項目＋CRM 12項目＝すべて合格）。
    - 見つけた1件（**F8: マニュアルの「動画で見る」が単元6 だけ無い**）を直し、「設定・ユーザー管理（管理者のみ）」の節を新設 → admin live `776931879c774899`（9/7 00:07 JST）
    - 🔴 **PMS: LPの問い合わせ内容が Formspree（外部・米国）にも送信されている**ことを確認。Cloud Functions と**同じ内容を二重に送っている**。本日のデプロイで**住所4項目が加わり、外部へ出る個人情報が増えた**。台帳・プライバシーポリシーの記載は未確認 → **止める／残して明記するの判断待ち**
    - 外部へ出る通信は4件（Formspree・zipcloud・Microsoft Clarity・GA4）。フォントは自己ホスト済みで0件
    - 引き継ぎリンク（`?t=`）と動画URLは**鍵を知っていれば読める**設計（leadTokens は `allow get: if true`・128bit・30日で失効。実測で符号なしは403・一覧不可）
    - 報告書は「本日のデプロイ分」に組み替え（17ページ・§1.5 デプロイ一覧／§3.8 総点検／§5.2b 外部送信の棚卸し／§5.2c 鍵の扱い）。ケアプー記録の項目は本報告書から除外
...
## メール送信元の切り替え（2026-09-08 夕方・次田さん＋藤田副理事長）

手順書: `docs/メール送信元の切り替え手順_20260908.md`（PDF は `00_outbox/介護情報基盤_メール送信元変更とPMS報告_20260908/01_…`）

- 送信は **Gmail API でそのユーザーとして送る**方式（`gmailAccessToken()` の `sub: sender`）。**エイリアス不可・実在アカウント必須**
- 同一ドメイン内なので **DWD の再登録は不要**。新アカウントに Gmail ライセンスが要る
- 設定画面で変えられるのは **差出人だけ**（`appConfig/settings.gmailSender`・デプロイ不要）
- 🔴 **開発側の修正が要るもの**（アドレスが決まってから）
  - **CC が `GMAIL_SENDER` 定数固定**（`functions/index.js` 576・720・866・907）→ 直さないと**控えが旧アドレスに届き続ける**。Functions の再デプロイが要る
  - **署名・問い合わせ先の直書き 31か所**（コード18・docs13。請求書・領収書・発注書・承諾書・認定書類・マニュアル）

## 問い合わせ通知を Formspree からメールへ切り替え（2026-09-09・本番反映済み）

- `functions/index.js` に `notifyInquiryMail()` を追加し、`webhookLpInquiry` の**新規・再問い合わせの両方**から呼ぶ。
  **宛先は設定の送信元アドレス（＝自分たちの受信箱）に固定**（公開エンドポイントなので、入力値を宛先にしない）
- 送信に失敗しても受付は止めない（Chat通知と同じく try/catch で握る）
- 🔴 **IAM を1つ足した**: `fn-webhook-sa` に `kjk-gmail-sa` の `roles/iam.serviceAccountTokenCreator`
  （webhookLpInquiry は SA_WEBHOOK で動いており Gmail 権限が無かった）
- `index.html`: form の `action`（Formspree）を削除し、成功判定を **webhook の応答（`hook.token`）**に変更。
  以前は **Formspree の応答で成功判定していた**ので、そこを変えないと送信後の画面が出ない
- 反映: Functions `webhookLpInquiry` 12:47 JST ／ lp live `c487d0c53affeddf` 12:55 JST
- 検証: プレビューで1件送信 → 案件登録・Chat通知・メール送信（エラーログ0）・引き継ぎリンク生成を確認。
  テストデータ（案件#149・事業所・記録・トークン）は削除済み。本番LPに `formspree.io` は**0件**

## 見積もりシステムの改修（藤田副理事長の提案・2026-09-09 一次対応を本番反映）

提案書: Google ドキュメント「20260907_介護情報基盤伴走支援 お見積りシステム改修提案書ver2」（藤田副理事長・2026-09-07）

**次田さんの判断（2026-09-09）**
1. 見積書は**ダウンロードとメールの両方**を用意し、利用者が選べるようにする
2. 見積もりは**CRMに登録する**（後から見返せるようにするため。提案書の「登録なし」案は採らない）
3. 表記は修正する。**申請は「支援」であり代行しない**。取引を煽る表現は使わない

**本番反映済み（lp live `57499c922ba1102b` ／ Functions `webhookMitsumori`・`sendQuotePdf`・`resendQuoteMail`）**
- 🔴 **同意チェックでは何も送信しなくなった**（REQ-01/02）。以前は `updateLock()` が自動で登録＋PDFメール送付まで走っていた
- 見積書のすぐ上に「**PDFでダウンロード**」「**メールで受け取る**」を新設（ヘッダにも同じ2つ）。
  `issueQuote(intent)` が唯一の送信トリガー。**どちらも案件として登録**し、**メールは「メールで受け取る」を押したときだけ**
- 見積書メールから「**2営業日以内にご連絡します**」を削除（REQ-03・§05）。返信で相談・申し込みを受ける文面に
- Chat通知に利用者の操作を明記（`INTENT_LABEL`：PDFをダウンロード／メールで受け取る／お申し込みの確認へ）
- 発行後、**申し込みの案内まで自動スクロール**（スマホでボタンに届かない問題・REQ-05）
- 表記統一: 申請代行 → **申請手続きの支援**／パック → **サービス内容・構成**／特別割引（出精値引き） → **金額調整**／
  伴走支援費の「1年間」表記を落とし、提供内容（初期登録・機器接続・アプリ設定・介護DX証明書設定・申請手続きの支援・導入後の運用相談）で説明
- 検証: プレビューで実測。**同意チェック後の送信0件** ／ 「メールで受け取る」→ 登録＋PDF送付（EST-2026-0016・`pdfPath` あり）／
  「PDFでダウンロード」→ 登録のみで**メールなし**（EST-2026-0017・`pdfPath` なし）。テストデータ（案件#150/#151・見積2件・事業所2件・記録3件・Storage PDF）は削除済み

### 会議（2026-09-09 19:00）で決めたいこと（未対応）
| # | 論点 | 状況 |
|---|---|---|
| 1 | **見積もり作成時の Chat 通知を止めるか**（提案書 REQ-03 は「見積のみは通知しない」） | いまは残している。操作の種別を通知文に出すところまで対応 |
| 2 | **LPの「1年間の伴走サポート」の表記**（4か所） | 見積書・フォーム側は落とした。LPは**提供期間の説明**なので、消すと支援の範囲が伝わらない。判断待ち |
| 3 | **「導入について相談する」ボタンの新設**（提案書 PHASE 1） | 未着手。LPの問い合わせフォームが同じ役割を担っている |
| 4 | **プライバシーポリシー・利用規約**（REQ-08） | **文面が無いと着手できない**。誰がいつまでに用意するかを決めたい |
| 5 | 申込確認画面の項目追加（REQ-06/07） | 現状でも確認画面はあるが、同意状態の表示などは未実装 |

## 見積もりシステムの改修 二次対応（2026-09-11・本番反映済み）

MTGの5論点に次田さんが回答（2026-09-11）。**5点すべて反映済み**。
admin live `74493cde303688de` ／ lp live `3a4b1cc19c6103ad` ／ Functions `webhookMitsumori`・`acceptQuote`

| # | 判断 | やったこと |
|---|---|---|
| 1 | 「設定を作って、今は止める」 | 設定画面に「**見積もりが作られたときも Chat に通知する**」を新設（`appConfig/settings.notifyQuoteChat`）。**既定＝オフ**（未設定はオフ扱い）。相談・申し込みの通知は従来どおり |
| 2 | 「それで良い。濁しておきたい」 | LPの「1年間の伴走サポート」4か所は**そのまま**（見積書・フォーム側は 9/9 に落とし済み）。⚠️ 「濁す＝現状維持」と解釈した。表現をやわらげる意図だったら直す |
| 3 | 「ボタンの文言を変える」 | 相談ボタンの新設はせず、**LP問い合わせフォームの送信ボタンを「無料で相談する」→「導入について相談する」**に変更（送信中→復帰の文言も同じ）。ページ内リンクのCTA4か所は未変更 |
| 4 | 「タダカヨのポリシーを確認してもらう」 | **所在を特定**: 個人情報保護方針 `https://tadakayo.jp/privacy-policy/`／利用規約 `https://tadakayo.jp/user-policy/`（いずれも令和8年9月1日改定）。LPのフッターと送信ボタン下、見積もり画面の同意欄、申込確認画面に導線を設置 |
| 5 | 「追加」 | 申込確認画面に**申し込む内容**（見積番号・プラン・カードリーダー構成・金額・自己負担・連絡先メール）を表示し、**「個人情報保護方針および利用規約に同意します」チェックを必須化**。同意した事実を `quotes.agreedPolicy` に記録 |

### ポリシー確認で分かったこと（要判断・PMS）
- タダカヨの個人情報保護方針には **Google アナリティクス・Meta ピクセル・Hotjar** と、**米国事業者の利用**が明記されている
- しかし **Microsoft Clarity の記載が無い**。介護情報基盤LPは Clarity を使っているので、**ポリシーに追記するか、Clarity をやめるか**の判断が要る
- zipcloud（郵便番号→住所）も記載が無い

## 申込モーダルの不具合 → 当日中に修正（2026-09-11・本番反映済み）

**上の ⑤ で申込確認画面に「申し込む内容」と同意チェックを足した結果モーダルが縦に伸び、
画面の低い環境で「この内容で申し込む」に届かなくなっていた。** 板垣さんが実際に止まり、
蜂須賀さんが同じ事象を再現して報告（Chat）。**私が前日に入れた変更が引き金の回帰。**

live `4828f4e0488c0262` ／ commit `627830e` ／ 変更は `mitsumori.html` の CSS のみ（HTML・JSは触っていない）

| 項目 | 修正前（本番実測 1280×500） | 修正後 |
|---|---|---|
| モーダルが画面内に収まるか | **いいえ**（上端 −291px／下端 791px） | はい（20〜480px） |
| 「この内容で申し込む」 | **画面外へ267pxはみ出す** | 画面内（416〜456px） |
| 箱をスクロールできるか | **できない**（overflow: visible） | できる（中身1095px） |
| ページをスクロールできるか | **できない** | — |
| ボタンの色 | 灰色 `rgb(239,239,239)` | タダカヨレッド `#E33535` |

直した3点
1. `.modal-box` に `max-height: calc(100dvh - 40px)` ＋ `overflow-y: auto` → 箱の中だけをスクロール
2. `.modal-actions` を `position: sticky; bottom: -24px` → スクロール前からボタンが見える
3. `.modal-btn-ok` / `.modal-btn-cancel` は**CSS定義が無く**両方灰色だったので色を当てた
   （定義されていたのは `-primary` / `-secondary` で、HTML側が使う名前と食い違っていた）

⚠️ **教訓: モーダルに要素を足したら、画面の低い環境（1280×500）でボタンに届くか必ず測る。**
overlay が `align-items: center` の設計では、中身が伸びた瞬間に箱の上下が画面外へ出て
**箱もページもスクロールできなくなる**。見た目の崩れではなく「操作不能」になるので気づきにくい。

### 板垣さんからの質問で未回答のもの
- 「メール返信で申し込むと良いのか」→ 画面が直ったので画面から申し込める。当座の回避は**ブラウザの縮小表示**（蜂須賀さんの案内どおりで正しい）
- **「領収書がないと助成金申請ができないのでここで止まっている」→ 支払い方法が「伴走支援の後にお支払い（請求書をお送りします）」のため、領収書がいつ出るのかの回答が要る。次田さんが晩にまとめて返信予定**
