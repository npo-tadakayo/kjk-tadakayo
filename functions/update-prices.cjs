// ⚠️【非推奨・再実行禁止】本スクリプトは旧「パススルー」前提（卸＝仕入と同額）の一回限り版。
//   2026-06-13 に方針変更：卸はパススルーではない（卸＝仕入＋一定卸益・数量帯別）。
//   正値は admin/js/pricing.js の PRODUCTS / Firestore appConfig.settings.partnerPricing が正。
//   再実行すると卸をパススルー値に戻してしまうため使わないこと。
// （旧説明）カードリーダー卸価格を AB Circle 正式回答(2026-06-05)へ更新する一回限りスクリプト。
